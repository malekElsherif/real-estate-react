...
const UserRequestHistory = () => {
  ...
  const cancelledOrRejectedRequests = requests.filter((r) => {
  ...
  if (requests.length === 0) {
    return (
      <div className="relative border border-[#14213D] bg-[#FFFDF9] p-12 text-center text-[#14213D]">
        ...
        <div className={`${mono} mx-auto flex h-12 w-12 items-center justify-center border border-[#14213D] bg-[#EFEAE0] text-sm font-semibold`}>
          00
        </div>
        ...
        <p className={`${mono} mt-2 text-xs uppercase tracking-wider text-[#4A5568]`}>
          You haven't submitted any property purchase requests yet.
        </p>
      </div>
    );
  }
  ...
  const RequestCard = ({ request }: { request: any }) => {
    const property = request.property ?? request;
      ...
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <h3 className={`${serif} truncate text-lg font-semibold text-[#14213D] transition-colors group-hover:text-[#B8863B]`}>
              {property.title || "Untitled Property"}
            </h3>

            <p className={`${mono} mt-1 text-xs text-[#4A5568]`}>
              LOC: {property.city || property.address || "Location Unspecified"}
            </p>

            {property.price && (
              <p className={`${mono} mt-2 text-xs font-semibold tracking-wider text-[#14213D]`}>
                VALUATION: {Number(property.price).toLocaleString()} EGP
              </p>
            )}

            {request.createdAt && (
              <p className={`${mono} mt-1 text-[11px] uppercase tracking-wider text-[#4A5568]/80`}>
  ...
  const tabs: { id: FilterTab; label: string; count: number }[] = [
    { id: "ALL", label: "All Logs", count: requests.length },
    { id: "APPROVED", label: "Approved", count: approvedRequests.length },
    { id: "PENDING", label: "Pending", count: pendingRequests.length },
    { id: "CANCELLED", label: "Cancelled & Rejected", count: cancelledOrRejectedRequests.length },
  ];
    ...
    <>
      ...
      <div className="relative border border-[#14213D] bg-[#FFFDF9] p-6 md:p-8">
        ...
        <div className="flex flex-col gap-4 border-b border-[#14213D] pb-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <span className={`${mono} text-xs font-semibold uppercase tracking-[0.2em] text-[#B8863B]`}>
              Portfolio Activity
            </span>
            <h2 className={`${serif} mt-1 text-2xl font-semibold text-[#14213D]`}>
              Purchase Requests
            </h2>
          </div>

          <span className={`${mono} inline-flex items-center border border-[#14213D] bg-[#EFEAE0] px-3 py-1 text-xs font-semibold uppercase tracking-wider text-[#14213D]`}>
        ...
        <div className="mt-6 space-y-8">
          ...
          {activeTab === "ALL" && (
            <div className="space-y-10">
              {approvedRequests.length > 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 border-b border-[#14213D]/20 pb-2">
                    ...
                    <h3 className={`${mono} text-xs font-semibold uppercase tracking-widest text-[#14213D]`}>
              ...
              {pendingRequests.length > 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 border-b border-[#14213D]/20 pb-2">
                    ...
                    <h3 className={`${mono} text-xs font-semibold uppercase tracking-widest text-[#B8863B]`}>
              ...
              {cancelledOrRejectedRequests.length > 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 border-b border-[#14213D]/20 pb-2">
                    <span className="h-2 w-2 bg-red-800" />
                    <h3 className={`${mono} text-xs font-semibold uppercase tracking-widest text-red-900`}>
                      Cancelled & Rejected ({cancelledOrRejectedRequests.length})
                    </h3>
                  </div>
                  <div className="space-y-3">
                    {cancelledOrRejectedRequests.map((req) => (
                      <RequestCard key={req.id} request={req} />
                    ))}
                  </div>
                </div>
          ...
          {activeTab === "APPROVED" && (
            <div className="space-y-3">
              {approvedRequests.length === 0 ? (
                <p className={`${mono} py-8 text-center text-xs uppercase text-[#4A5568]`}>
          ...
          {activeTab === "CANCELLED" && (
            <div className="space-y-3">
              {cancelledOrRejectedRequests.length === 0 ? (
                <p className={`${mono} py-8 text-center text-xs uppercase text-[#4A5568]`}>
                  No cancelled or rejected requests found.
                </p>
              ) : (
                cancelledOrRejectedRequests.map((req) => <RequestCard key={req.id} request={req} />)
              )}
            </div>
          )}
        </div>
      ...
      >
        <DialogContent className="!p-0">
          {selectedRequest && (() => {
            const property = selectedRequest.property ?? selectedRequest;
            const status = selectedRequest.status?.toLowerCase() ?? "pending";
            ...
            return (
              <div className="relative text-[#14213D]">
                ...
                <div className="border-b border-[#14213D] bg-[#14213D] px-8 py-6 text-[#F7F5EF]">
                  <span className={`${mono} text-xs uppercase tracking-widest text-[#B8863B]`}>
                    Request Reference #{selectedRequest.id}
                  </span>

                  <h2 className={`${serif} mt-2 pr-8 text-2xl font-semibold`}>
                    {property.title || "Property Details"}
                  </h2>

                  <p className={`${mono} mt-1 text-xs text-[#EFEAE0]`}>
                    LOC: {property.city || property.address || "Location Unspecified"}
                  </p>
                </div>
                ...
                <div className="space-y-6 p-8">
                  <div className="border border-[#14213D] bg-[#F7F5EF] p-4">
                    <p className={`${mono} text-[11px] font-semibold uppercase tracking-widest text-[#4A5568]`}>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {property.price && (
                      <div className="border border-[#14213D]/20 p-4">
                        ...
                        <p className={`${serif} mt-1 text-lg font-semibold text-[#14213D]`}>
                    ...
                    <div className="border border-[#14213D]/20 p-4">
                      ...
                      <p className="mt-1 text-xs font-medium text-[#14213D]">
                        {property.address || property.city || "Not available"}
                      </p>
                    </div>
                    ...
                    {selectedRequest.createdAt && (
                      <div className="border border-[#14213D]/20 p-4 sm:col-span-2">
                        ...
                        <p className={`${mono} mt-1 text-xs text-[#14213D]`}>
